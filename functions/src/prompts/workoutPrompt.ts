...
export function buildUserPrompt(context: WorkoutContext): string {
  const parts: string[] = [];
...
- Weight unit preference: ${context.weightUnit}`);
  ...
  if (context.recentWorkouts.length > 0) {
    const recentLines = context.recentWorkouts
      .map(
        (w) =>
          `  - ${w.date}: ${w.focus} (${w.durationMinutes}min, ${w.exercises.length} exercises)`
      )
      .join("\n");
  ...
  if (context.activeInjuries.length > 0) {
    const injuryLines = context.activeInjuries
      .map(
        (i) =>
          `  - ${i.location} (${i.phase} phase): avoid ${i.restrictions.join(", ")}`
      )
      .join("\n");
    parts.push(`\nACTIVE INJURIES:\n${injuryLines}`);
  }
...
function getExerciseCount(minutes: number): string {
  if (minutes <= 30) return "4-5";
  if (minutes <= 45) return "5-7";
  if (minutes <= 60) return "6-8";
  return "7-10";
}
