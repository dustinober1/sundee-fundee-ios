...
import 'package:sundee_fundee/shared/providers/connectivity_provider.dart';
...
void main() {
  ...
  group('CHRT-01: Progress screen renders all 3 chart sections', () {
    testWidgets('progress screen shows 1RM chart, volume chart, and heatmap', (tester) async {
      ...
      final progressTab = find.byIcon(Icons.insights);
  ...
  group('CHRT-02: Progress screen loads from local Drift while offline', () {
    testWidgets('progress data renders without network connectivity', (tester) async {
      ...
      // Force offline
      final fakeConnectivity = FakeConnectivityPlatform([ConnectivityResult.none]);
...
- FakeConnectivityPlatform for offline test (existing helper)
- Unit tests for calculations.dart (fast, no tester needed)
- Integration tests for plateau_detection.dart (needs real Drift db)
- **Weight PR integration tests verify correct ordering (PR check BEFORE 1RM save)**
- Import WorkoutRepository for weight PR tests
  </action>
  <verify>
    ...
    `grep "FakeConnectivityPlatform" flutter_app/integration_test/parity_gates/recommendations_parity_test.dart` — Offline test pattern.
    `grep "checkAndSaveWeightPR" flutter_app/integration_test/parity_gates/recommendations_parity_test.dart` — Weight PR tests present.
  </verify>
  <done>
...
2. `grep -c "group\|testWidgets\|test(" flutter_app/integration_test/parity_gates/recommendations_parity_test.dart` — Multiple test cases
3. `grep "recommendations" flutter_app/integration_test/all_tests.dart` — Aggregator updated
</verification>

<success_criteria>
- [ ] recommendations_parity_test.dart covers CHRT-01, CHRT-02, RECO-01, RECO-02
- [ ] CHRT-01: Tests 3 chart Key selectors present
- [ ] CHRT-02: Tests progress loads with FakeConnectivityPlatform offline
