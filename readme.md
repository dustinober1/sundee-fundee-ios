...
- 🗑️ **History Management:** Users can now delete workout history and period logs with confirmation, and view detailed workout summaries from their history.
...
- **Firestore Missing Index Error**: If you see `[cloud_firestore/failed-precondition] The query requires an index`, ensure that `firestore.indexes.json` includes `queryScope: COLLECTION` indices for subcollections. `COLLECTION_GROUP` query scopes do not satisfy `COLLECTION` scoped queries in all Firestore conditions. The fix requires duplicating the index fields in `firestore.indexes.json` with the alternative scope and deploying via `firebase deploy --only firestore:indexes`.
- **Dart Compiler Errors**: Resolved duplicate `_` variable naming errors in GoRouter error handlers and cycle tracking error states. Fixed missing required named parameters in `ProgramWeek` and `ProgramExercise` constructors in the admin dashboard. Addressed type resolution issues with Riverpod `AutoDisposeNotifier` in the rest timer provider by switching to `Notifier` with `.autoDispose` provider factory. Fixed missing `go_router` import in `sign_in_screen.dart` which caused `context.push` to be undefined during hot reload. Resolved `use_build_context_synchronously` lint in the admin dashboard and silenced `avoid_print` warnings in benchmark tests.
