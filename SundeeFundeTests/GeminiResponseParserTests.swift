...
struct GeminiResponseParserTests {
    ...
    // MARK: - Valid Response Tests (new weight + weightUnit fields)

    @Test func parsesValidResponseWithWeightedExerciseKg() throws {
        ...
        #expect(exercise.weightUnit == "kg")
        // Weight should be a kg value close to 60 (may be snapped)
    ...
    @Test func parsesValidResponseWithWeightedExerciseLb() throws {
    ...
    // MARK: - Backward Compatibility (legacy weightLb field)

    @Test func parsesLegacyWeightLbField() throws {
        let exercises: [[String: Any]] = [
            [
                "name": "Barbell Squat",
                "sets": 4,
        ...
        let exercise = workout.exercises[0]
        // Legacy response parsed with weightLb should default to "lb" unit
        #expect(exercise.weightUnit == "lb")
        #expect(exercise.weight == 135.0)
    }

    @Test func prefersNewWeightFieldOverLegacyWeightLb() throws {
        // If both fields present, new `weight` should take priority
        let exercises: [[String: Any]] = [
            [
                "name": "Barbell Squat",
                "sets": 4,
        ...
        #expect(exercise.weightUnit == "kg")
        #expect(exercise.weight != nil)
        // Should use the kg value (60), not the lb value (135)
        // (may be snapped but should be in kg range)
        if let w = exercise.weight {
            #expect(w < 100, "Should be kg range (~60), not lb range (135)")
        }
    }
