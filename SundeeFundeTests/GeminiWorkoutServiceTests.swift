...
struct GeminiWorkoutServiceTests {
    ...
    func successfulGeneration() async throws {
        ...
        // Legacy weightLb field in response is parsed via backward-compat path → weight == 80, weightUnit == "lb"
        #expect(workout.exercises[0].weight == 80)
        #expect(workout.exercises[0].weightUnit == "lb")
