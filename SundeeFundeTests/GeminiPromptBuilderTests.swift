...
struct GeminiPromptBuilderTests {
    ...
    // MARK: - System Prompt (Unit-Parameterized)

    @Test func systemPromptIsNotEmpty() {
        #expect(!GeminiPromptBuilder.systemPrompt(weightUnit: "lb").isEmpty)
        #expect(!GeminiPromptBuilder.systemPrompt(weightUnit: "kg").isEmpty)
    }

    @Test func systemPromptLbsContainsCoachingGuidance() {
        let prompt = GeminiPromptBuilder.systemPrompt(weightUnit: "lb")
    ...
    @Test func systemPromptKgContainsCoachingGuidance() {
        let prompt = GeminiPromptBuilder.systemPrompt(weightUnit: "kg")
        #expect(prompt.contains("compound movements"))
        #expect(prompt.contains("injury restrictions"))
        #expect(prompt.contains("menstrual cycle phase"))
        #expect(prompt.contains("energy level"))
    ...
    @Test func systemPromptKgUsesMensKgBar() {
        let prompt = GeminiPromptBuilder.systemPrompt(weightUnit: "kg")
        #expect(prompt.contains("20 kg bar"))
        #expect(!prompt.contains("45 lb bar"))
    }

    @Test func systemPromptLbsUsesMensLbBar() {
        let prompt = GeminiPromptBuilder.systemPrompt(weightUnit: "lb")
        #expect(prompt.contains("45 lb bar"))
        #expect(!prompt.contains("20 kg bar"))
    }

    @Test func systemPromptKgUsesKgEquipmentOnly() {
        let prompt = GeminiPromptBuilder.systemPrompt(weightUnit: "kg")
        // Should mention kg plates and not lb plates
        #expect(prompt.contains("KILOGRAMS"))
        #expect(!prompt.contains("POUNDS"))
        #expect(!prompt.contains("45 lb bar"))
    }

    @Test func systemPromptLbsUsesLbEquipmentOnly() {
        let prompt = GeminiPromptBuilder.systemPrompt(weightUnit: "lb")
        // Should mention lbs and not kg bar
        #expect(prompt.contains("POUNDS"))
        #expect(!prompt.contains("KILOGRAMS"))
        #expect(!prompt.contains("20 kg bar"))
    }
    ...
    // MARK: - Maxes (unit-aware conversion)

    @Test func userPromptIncludesMaxesInLbs() {
        let maxes = [
            ExerciseMax(name: "Back Squat", weightLb: 100),
            ExerciseMax(name: "Bench Press", weightLb: 80)
        ]
        let context = makeContext(maxes: maxes, weightUnit: "lb")
        let prompt = GeminiPromptBuilder.userPrompt(from: context)
        #expect(prompt.contains("Back Squat"))
        #expect(prompt.contains("100"))
        #expect(prompt.contains("Bench Press"))
        #expect(prompt.contains("80"))
        #expect(prompt.contains("lb"))
        #expect(prompt.contains("Prefer programming exercises the athlete has maxes for"))
    }

    @Test func userPromptConvertsMaxesToKgWhenUnitIsKg() {
        // 225 lb squat should convert to approximately 102 kg
        let maxes = [ExerciseMax(name: "Back Squat", weightLb: 225)]
        let context = makeContext(maxes: maxes, weightUnit: "kg")
        let prompt = GeminiPromptBuilder.userPrompt(from: context)
        #expect(prompt.contains("Back Squat"))
        // 225 / 2.2046 ≈ 102 kg
        #expect(prompt.contains("102"))
        #expect(prompt.contains("kg"))
        #expect(!prompt.contains("225 lb"))
    }

    @Test func userPromptKeepsLbMaxesWhenUnitIsLb() {
        let maxes = [ExerciseMax(name: "Back Squat", weightLb: 225)]
        let context = makeContext(maxes: maxes, weightUnit: "lb")
        let prompt = GeminiPromptBuilder.userPrompt(from: context)
        #expect(prompt.contains("225"))
        #expect(prompt.contains("lb"))
    }
    ...
    // MARK: - Body Weight (unit-aware)

    @Test func userPromptBodyWeightConvertedToKgWhenUnitIsKg() {
        // 70 kg body weight should appear as 70 kg
        let context = makeContext(weightUnit: "kg", bodyWeightKg: 70.0)
        let prompt = GeminiPromptBuilder.userPrompt(from: context)
        #expect(prompt.contains("70 kg") || prompt.contains("70"))
        #expect(prompt.contains("kg"))
    }

    @Test func userPromptBodyWeightConvertedToLbWhenUnitIsLb() {
        // 70 kg ≈ 154 lb
        let context = makeContext(weightUnit: "lb", bodyWeightKg: 70.0)
        let prompt = GeminiPromptBuilder.userPrompt(from: context)
        #expect(prompt.contains("154") || prompt.contains("lb"))
    }
    ...
    @Test func responseSchemaExercisesIsArray() {
        ...
        // Must use new weight + weightUnit fields (not weightLb)
        #expect(itemProps?["weight"] != nil)
        #expect(itemProps?["weightUnit"] != nil)
    ...
    @Test func responseSchemaDoesNotContainWeightLbField() {
        let schema = GeminiPromptBuilder.responseSchema
        let properties = schema["properties"] as? [String: Any]
        let exercises = properties?["exercises"] as? [String: Any]
        let items = exercises?["items"] as? [String: Any]
