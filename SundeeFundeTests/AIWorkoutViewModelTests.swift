...
struct WorkoutPreviewViewModelTests {
    ...
    private func makeSampleWorkout() -> GeneratedWorkout {
        GeneratedWorkout(
            ...
            exercises: [
                GeneratedExercise(id: "ex-1", name: "Back Squat", sets: 4, reps: "5", weight: 100, weightUnit: "lb", restMinutes: 3.0),
                GeneratedExercise(id: "ex-2", name: "Bench Press", sets: 3, reps: "8", weight: 80, weightUnit: "lb", restMinutes: 2.0),
    ...
    @Test @MainActor func updateWeight() {
        ...
        #expect(vm.workout.exercises.first { $0.id == "ex-1" }?.weight == 110)
    }
    ...
    @Test @MainActor func updateWeightNonExistentID() {
        ...
        #expect(vm.workout.exercises.first?.weight == 100)
    }
    ...
    @Test @MainActor func exerciseSummaryWithWeightLb() {
        let ex = GeneratedExercise(name: "Squat", sets: 4, reps: "5", weight: 225, weightUnit: "lb")
    ...
    @Test @MainActor func exerciseSummaryWithWeightKg() {
        let ex = GeneratedExercise(name: "Squat", sets: 4, reps: "5", weight: 220, weightUnit: "lb")
        let summary = WorkoutPreviewViewModel.exerciseSummary(ex, weightUnit: .kilograms)
        // 220 lb ≈ 99.8 kg → displays as "100 kg"
        #expect(summary.hasPrefix("4 x 5 @"))
    ...
    @Test @MainActor func exerciseSummaryZeroWeight() {
        let ex = GeneratedExercise(name: "Test", sets: 3, reps: "10", weight: 0, weightUnit: "lb")
        let summary = WorkoutPreviewViewModel.exerciseSummary(ex, weightUnit: .pounds)
