...
// MARK: - DailyCoachingRecommendation

/// A persisted daily coaching decision that combines recovery, cycle phase,
/// training load, pain, and coach insights into a single actionable state.
public struct DailyCoachingRecommendation: Codable, Sendable, Identifiable, Equatable {
    ...
    public var status: DailyCoachingStatus {
        DailyCoachingStatus(rawValue: statusRaw) ?? .maintain
    }
}

// MARK: - DailyCoachingStatus

public enum DailyCoachingStatus: String, Codable, Sendable, CaseIterable {
    case push
    case build
    case maintain
    case deload
...
// MARK: - DailyCoachingPrimaryAction

public enum DailyCoachingPrimaryAction: String, Codable, Sendable, CaseIterable {
    case startAdjustedWorkout
    case viewRecoveryDetails
    case takeActiveRecovery
    case rest
}
