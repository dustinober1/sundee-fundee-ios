...
enum AppModelContainer {
    ...
    static let shared: ModelContainer = {
        ...
        // Tier 1 — CloudKit-backed persistent store.
        // Note: @Attribute(.unique) is NOT used on any model because CloudKit does not
        // support unique constraints. Uniqueness is enforced at the application layer
        // via UUID-based id generation.
        do {
            let schema = Schema(allModels)
        ...
        // Used when CloudKit is unavailable (no entitlements, no iCloud account, offline).
        do {
            let schema = Schema(allModels)
