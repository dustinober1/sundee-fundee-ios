...
Project-local skills live in `.claude/skills/`. Notable:
- `cloudkit-validate` — validates Swift models against the CloudKit schema rules above.
- `asc-*` family — App Store Connect workflows (submission, metadata, TestFlight, screenshots, signing, pricing, etc.). Discover via `ls .claude/skills/` when an ASC task comes up.
